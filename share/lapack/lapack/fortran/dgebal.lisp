  ...
  (defun dgebal (job n a lda ilo ihi scale info)
    ...
    (f2cl-lib:with-multi-array-data
      ...
      (prog ((c 0.0) (ca 0.0) (f 0.0) (g 0.0) (r 0.0) (ra 0.0) (s 0.0)
        ...
        (declare
         (type (double-float) c ca f g r ra s sfmax1 sfmax2 sfmin1 sfmin2)
         (type (f2cl-lib:integer4) i ica iexc ira j k l m)
         (type f2cl-lib:logical noconv))
        (setf info 0)
        (cond
         ((and (not (lsame job "N")) (not (lsame job "P"))
         ...
         ((< lda (max (the f2cl-lib:integer4 1) (the f2cl-lib:integer4 n)))
          (setf info -4)))
        (cond
         ((/= info 0) (xerbla "DGEBAL" (f2cl-lib:int-sub info))
          (go end_label)))
        (setf k 1)
        (setf l n)
        (if (= n 0)
       ...
       label20
        ...
        (if (= j m)
            (go label30))
        (dswap l
         (f2cl-lib:array-slice a-%data% double-float (1 j) ((1 lda) (1 *))
                               a-%offset%)
         1
         (f2cl-lib:array-slice a-%data% double-float (1 m) ((1 lda) (1 *))
                               a-%offset%)
         1)
        (dswap (f2cl-lib:int-add (f2cl-lib:int-sub n k) 1)
         (f2cl-lib:array-slice a-%data% double-float (j k) ((1 lda) (1 *))
                               a-%offset%)
         lda
         (f2cl-lib:array-slice a-%data% double-float (m k) ((1 lda) (1 *))
                               a-%offset%)
         lda)
       label30
          ...
          (tagbody
            ...
            (setf ira
                    (idamax (f2cl-lib:int-add (f2cl-lib:int-sub n k) 1)
                     (f2cl-lib:array-slice a-%data% double-float (i k)
                                           ((1 lda) (1 *)) a-%offset%)
                     lda))
            (setf ra
                    (abs
           ...
           label190
            ...
            (cond
             ((and (> f one) (> (f2cl-lib:fref scale (i) ((1 *))) one))
              (if (>= (f2cl-lib:fref scale-%data% (i) ((1 *)) scale-%offset%)
                      (/ sfmax1 f))
                  (go label200))))
            (setf g (/ one f))
            (setf (f2cl-lib:fref scale-%data% (i) ((1 *)) scale-%offset%)
            ...
            (dscal (f2cl-lib:int-add (f2cl-lib:int-sub n k) 1) g
             (f2cl-lib:array-slice a-%data% double-float (i k) ((1 lda) (1 *))
                                   a-%offset%)
             lda)
            (dscal l f
             (f2cl-lib:array-slice a-%data% double-float (1 i) ((1 lda) (1 *))
                                   a-%offset%)
             1)
           label200))
        (if noconv
