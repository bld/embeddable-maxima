    ...
    (f2cl-lib:with-multi-array-data
      ...
      (prog ((i 0) (lastv 0) (lastc 0) (applyleft nil))
        ...
        (cond
         ((/= tau zero)
          (tagbody
            (cond (applyleft (setf lastv m)) (t (setf lastv n)))
            (cond
             ((> incv 0)
              (setf i
                      (f2cl-lib:int-add 1
                                        (f2cl-lib:int-mul
                                         (f2cl-lib:int-sub lastv 1) incv))))
             (t (setf i 1)))
           label100000
            (if (not
                 (and (> lastv 0)
             ...
             (applyleft
              (setf lastc
                      (multiple-value-bind (ret-val var-0 var-1 var-2 var-3)
                        ...
                        (when var-0 (setf lastv var-0))
                        (when var-1 (setf n var-1))
                        (when var-3 (setf ldc var-3))
                        ret-val)))
             (t
              (setf lastc
                      (multiple-value-bind (ret-val var-0 var-1 var-2 var-3)
