...
(let* ((zero 0.0) (half 0.5) (one 1.0) (multpl 4.0))
  ...
  (defun dlanv2 (a b c d rt1r rt1i rt2r rt2i cs sn)
    ...
    (prog ((aa 0.0) (bb 0.0) (bcmax 0.0) (bcmis 0.0) (cc 0.0) (cs1 0.0)
      ...
      (declare
       (type (double-float) aa bb bcmax bcmis cc cs1 dd eps p sab sac scale
        sigma sn1 tau temp z))
      (setf eps (dlamch "P"))
      (cond ((= c zero) (setf cs one) (setf sn zero) (go label10))
            ((= b zero) (setf cs zero) (setf sn one) (setf temp d) (setf d a)
             (setf a temp) (setf b (- c)) (setf c zero) (go label10))
            ((and (= (+ a (- d)) zero)
                  (/= (f2cl-lib:sign one b) (f2cl-lib:sign one c)))
             (setf cs one) (setf sn zero) (go label10))
            (t (setf temp (- a d)) (setf p (* half temp))
             (setf bcmax (max (abs b) (abs c)))
             (setf bcmis
                     (* (min (abs b) (abs c)) (f2cl-lib:sign one b)
                        (f2cl-lib:sign one c)))
             ...
             (cond
              ((>= z (* multpl eps))
               (setf z
                       (+ p
                          (f2cl-lib:sign
                           (* (f2cl-lib:fsqrt scale) (f2cl-lib:fsqrt z)) p)))
               (setf a (+ d z)) (setf d (- d (* (/ bcmax z) bcmis)))
               (setf tau (dlapy2 c z)) (setf cs (/ z tau)) (setf sn (/ c tau))
               (setf b (- b c)) (setf c zero))
              (t (setf sigma (+ b c)) (setf tau (dlapy2 sigma temp))
               ...
               (setf c (+ (* (- aa) sn) (* cc cs)))
               (setf d (+ (* (- bb) sn) (* dd cs)))
               (setf temp (* half (+ a d))) (setf a temp) (setf d temp)
               (cond
                ((/= c zero)
                 (cond
                  ((/= b zero)
                   (cond
                    ((= (f2cl-lib:sign one b) (f2cl-lib:sign one c))
                     ...
                     (setf a (+ temp p)) (setf d (- temp p)) (setf b (- b c))
                     (setf c zero) (setf cs1 (* sab tau))
                     (setf sn1 (* sac tau))
                     (setf temp (- (* cs cs1) (* sn sn1)))
                     (setf sn (+ (* cs sn1) (* sn cs1))) (setf cs temp))))
                  (t (setf b (- c)) (setf c zero) (setf temp cs)
                   (setf cs (- sn)) (setf sn temp)))))))))
     label10
      (setf rt1r a)
      (setf rt2r d)
      (cond ((= c zero) (setf rt1i zero) (setf rt2i zero))
            (t
  ...
  (setf (gethash 'fortran-to-lisp::dlanv2
          ...
          (fortran-to-lisp::make-f2cl-finfo :arg-types
                                            ...
                                            '(fortran-to-lisp::a
                                              ...
                                              fortran-to-lisp::c
                                              fortran-to-lisp::d
                                              fortran-to-lisp::rt1r
                                              fortran-to-lisp::rt1i
                                              fortran-to-lisp::rt2r
                                              fortran-to-lisp::rt2i
                                              fortran-to-lisp::cs
                                              fortran-to-lisp::sn)
                                            :calls
