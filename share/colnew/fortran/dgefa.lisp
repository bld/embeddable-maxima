        ...
        (tagbody
                  ...
                  (f2cl-lib:int-sub
                   (f2cl-lib:int-add
                    (idamax (f2cl-lib:int-add (f2cl-lib:int-sub n k) 1)
                     (f2cl-lib:array-slice a-%data% double-float (k k)
                                           ((1 lda) (1 1)) a-%offset%)
                     1)
                    k)
                   1))
         ...
         label10
          ...
          (f2cl-lib:fdo (j kp1 (f2cl-lib:int-add j 1))
            ...
            (tagbody
              (setf t$
                      (f2cl-lib:fref a-%data% (l j) ((1 lda) (1 1))
                                     a-%offset%))
              (if (= l k)
                  (go label20))
              (setf (f2cl-lib:fref a-%data% (l j) ((1 lda) (1 1)) a-%offset%)
                      (f2cl-lib:fref a-%data% (k j) ((1 lda) (1 1))
                                     a-%offset%))
             ...
             label20
              (daxpy (f2cl-lib:int-sub n k) t$
               (f2cl-lib:array-slice a-%data% double-float ((+ k 1) k)
                                     ((1 lda) (1 1)) a-%offset%)
               1
               (f2cl-lib:array-slice a-%data% double-float ((+ k 1) j)
                                     ((1 lda) (1 1)) a-%offset%)
               1)
             label30))
          (go label50)
