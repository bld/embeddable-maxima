...
(let ((zeror 0.0)
      ...
      (con
       (make-array 2 :element-type 'double-float :initial-contents
                   '(0.3989422804014327 1.2533141373155003)))
      (c
       (make-array 120 :element-type 'double-float :initial-contents
                   '(1.0 -0.20833333333333334 0.125 0.3342013888888889
                     ...
                     -3871833.442572613 18257.755474293175 2.86464035717679e11
                     -2.406297900028504e12 9.109341185239898e12
                     -2.0516899410934438e13 3.056512551993532e13
                     -3.166708858478516e13 2.334836404458184e13
                     -1.2320491305598287e13 4.612725780849132e12
                     -1.1965528801961816e12 2.0591450323241e11
                     -2.1822927757529224e10 1.2470092935127103e9
                     -2.9188388122220814e7 118838.42625678325))))
  (declare (type (double-float) zeror zeroi coner conei)
           (type (array double-float (2)) con)
           (type (array double-float (120)) c))
  (defun zunik
         (zrr zri fnu ikflg ipmtr tol init phir phii zeta1r zeta1i zeta2r
          zeta2i sumr sumi cwrkr cwrki)
    (declare (type (array double-float (*)) cwrki cwrkr)
    ...
    (f2cl-lib:with-multi-array-data
        ((cwrkr double-float cwrkr-%data% cwrkr-%offset%)
         (cwrki double-float cwrki-%data% cwrki-%offset%))
      (prog ((i 0) (idum 0) (j 0) (k 0) (l 0) (ac 0.0) (crfni 0.0) (crfnr 0.0)
             (rfn 0.0) (si 0.0) (sr 0.0) (sri 0.0) (srr 0.0) (sti 0.0)
             (str 0.0) (test 0.0) (ti 0.0) (tr 0.0) (t2i 0.0) (t2r 0.0)
             (zni 0.0) (znr 0.0))
        (declare
       ...
       label15
        ...
        (multiple-value-bind (var-0 var-1 var-2 var-3)
          ...
          (declare (ignore var-0 var-1))
          (setf (f2cl-lib:fref cwrkr-%data% (16) ((1 16)) cwrkr-%offset%)
                  var-2)
          (setf (f2cl-lib:fref cwrki-%data% (16) ((1 16)) cwrki-%offset%)
                  var-3))
        (setf phir
                (* (f2cl-lib:fref cwrkr-%data% (16) ((1 16)) cwrkr-%offset%)
        ...
        (multiple-value-bind (var-0 var-1 var-2 var-3 var-4 var-5)
          ...
          (setf t2r var-4)
          (setf t2i var-5))
        (setf (f2cl-lib:fref cwrkr-%data% (1) ((1 16)) cwrkr-%offset%) coner)
        (setf (f2cl-lib:fref cwrki-%data% (1) ((1 16)) cwrki-%offset%) conei)
        (setf crfnr coner)
        (setf crfni conei)
        (setf ac 1.0)
        (setf l 1)
        (f2cl-lib:fdo (k 2 (f2cl-lib:int-add k 1))
          ...
          (tagbody
            ...
            (setf crfnr str)
            (setf (f2cl-lib:fref cwrkr-%data% (k) ((1 16)) cwrkr-%offset%)
                    (- (* crfnr sr) (* crfni si)))
            (setf (f2cl-lib:fref cwrki-%data% (k) ((1 16)) cwrki-%offset%)
                    (+ (* crfnr si) (* crfni sr)))
            (setf ac (* ac rfn))
            (setf test
                    (+
                     ...
                     (abs
                      (f2cl-lib:fref cwrki-%data% (k) ((1 16))
                                     cwrki-%offset%))))
            (if (and (< ac tol) (< test tol))
                (go label30))
           label20))
        (setf k 15)
       label30
       ...
       label40
        ...
        (f2cl-lib:fdo (i 1 (f2cl-lib:int-add i 1))
          ...
          (tagbody
            (setf sr
                    (+ sr
                       (f2cl-lib:fref cwrkr-%data% (i) ((1 16))
                                      cwrkr-%offset%)))
            (setf si
                    (+ si
                       (f2cl-lib:fref cwrki-%data% (i) ((1 16))
                                      cwrki-%offset%)))
           label50))
        (setf sumr sr)
        (setf sumi si)
       ...
       end_label
        (return
         (values nil nil nil nil nil nil init phir phii zeta1r zeta1i zeta2r
                 zeta2i sumr sumi nil nil))))))
  ...
  (setf (gethash 'fortran-to-lisp::zunik fortran-to-lisp::*f2cl-function-info*)
          (fortran-to-lisp::make-f2cl-finfo :arg-types
                                            ...
                                            '(nil nil nil nil nil nil
                                              ...
                                              fortran-to-lisp::sumr
                                              fortran-to-lisp::sumi nil nil)
                                            :calls
