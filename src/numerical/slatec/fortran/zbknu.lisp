...
(let ((kmax 30)
      ...
      (cc
       (make-array 8 :element-type 'double-float :initial-contents
                   '(0.5772156649015329 -0.04200263503409524
                     -0.04219773455554433 0.0072189432466631
                     -2.1524167411495098e-4 -2.013485478078824e-5
                     1.133027231981696e-6 6.116095104481416e-9))))
  (declare (type (f2cl-lib:integer4) kmax)
           (type (double-float) czeror czeroi coner conei ctwor r1 dpi rthpi
            spi hpi fpi tth)
           (type (array double-float (8)) cc))
  (defun zbknu (zr zi fnu kode n yr yi nz tol elim alim)
    ...
    (f2cl-lib:with-multi-array-data
      ...
      (prog ((cssr (make-array 3 :element-type 'double-float))
             ...
             (fi 0.0) (fk 0.0) (fks 0.0) (fmui 0.0) (fmur 0.0) (fr 0.0)
             (g1 0.0) (g2 0.0) (pi$ 0.0) (pr 0.0) (pti 0.0) (ptr 0.0) (p1i 0.0)
             (p1r 0.0) (p2i 0.0) (p2m 0.0) (p2r 0.0) (qi 0.0) (qr 0.0)
             (rak 0.0) (rcaz 0.0) (rzi 0.0) (rzr 0.0) (s 0.0) (smui 0.0)
             (smur 0.0) (sti 0.0) (str 0.0) (s1i 0.0) (s1r 0.0) (s2i 0.0)
             (s2r 0.0) (tm 0.0) (t1 0.0) (t2 0.0) (elm 0.0) (celmr 0.0)
             (zdr 0.0) (zdi 0.0) (as 0.0) (alas 0.0) (helim 0.0))
        (declare (type (array double-float (2)) cyi cyr)
                 ...
                 (type (double-float) helim alas as zdi zdr celmr elm t2 t1 tm
                  s2r s2i s1r s1i str sti smur smui s rzr rzi rcaz rak qr qi
                  p2r p2m p2i p1r p1i ptr pti pr pi$ g2 g1 fr fmur fmui fks fk
                  fi fhs fc etest dnu2 dnu czr czi csr csi cshr cshi csclr
                  crscr coefr coefi ckr cki cchr cchi cbr cbi caz bk bb a2 a1
                  ascle ak aa)
                 (type (f2cl-lib:integer4) nw inub ic j idum koded kk kflag k
                  inu iflag i))
       ...
       label60
        ...
        (if (> a1 tol)
            (go label60))
       label70
        (setf (f2cl-lib:fref yr-%data% (1) ((1 n)) yr-%offset%) s1r)
        (setf (f2cl-lib:fref yi-%data% (1) ((1 n)) yi-%offset%) s1i)
        (if (= koded 1)
            (go end_label))
        (multiple-value-bind (var-0 var-1 var-2 var-3)
            (zexp zr zi str sti)
          ...
          (setf sti var-3))
        (multiple-value-bind (var-0 var-1 var-2 var-3 var-4 var-5)
            (zmlt s1r s1i str sti
             (f2cl-lib:fref yr-%data% (1) ((1 n)) yr-%offset%)
             (f2cl-lib:fref yi-%data% (1) ((1 n)) yi-%offset%))
          (declare (ignore var-0 var-1 var-2 var-3))
          (setf (f2cl-lib:fref yr-%data% (1) ((1 n)) yr-%offset%) var-4)
          (setf (f2cl-lib:fref yi-%data% (1) ((1 n)) yi-%offset%) var-5))
        (go end_label)
       label80
        (if (< caz tol)
            (go label100))
       ...
       label210
        ...
        (setf cki (* str rzi))
        (if (= n 1)
            (setf inu (f2cl-lib:int-sub inu 1)))
        (if (> inu 0)
            (go label220))
        (if (> n 1)
            (go label215))
       ...
       label240
        ...
        (setf (f2cl-lib:fref yr-%data% (1) ((1 n)) yr-%offset%) (* s1r str))
        (setf (f2cl-lib:fref yi-%data% (1) ((1 n)) yi-%offset%) (* s1i str))
        (if (= n 1)
            (go end_label))
        (setf (f2cl-lib:fref yr-%data% (2) ((1 n)) yr-%offset%) (* s2r str))
        (setf (f2cl-lib:fref yi-%data% (2) ((1 n)) yi-%offset%) (* s2i str))
        (if (= n 2)
            (go end_label))
        (setf kk 2)
       label250
        ...
        (f2cl-lib:fdo (i kk (f2cl-lib:int-add i 1))
          ...
          (tagbody
            ...
            (setf p2r (* s2r p1r))
            (setf p2i (* s2i p1r))
            (setf (f2cl-lib:fref yr-%data% (i) ((1 n)) yr-%offset%) p2r)
            (setf (f2cl-lib:fref yi-%data% (i) ((1 n)) yi-%offset%) p2i)
            (if (>= kflag 3)
                (go label260))
            (setf str (abs p2r))
            (setf sti (abs p2i))
       ...
       label270
        ...
        (if (= n 1)
            (go label280))
        (setf (f2cl-lib:fref yr-%data% (2) ((1 n)) yr-%offset%) s2r)
        (setf (f2cl-lib:fref yi-%data% (2) ((1 n)) yi-%offset%) s2i)
       label280
        (setf ascle (f2cl-lib:fref bry (1) ((1 3))))
        (multiple-value-bind
        ...
        (setf s2r (f2cl-lib:fref yr-%data% (kk) ((1 n)) yr-%offset%))
        (setf s2i (f2cl-lib:fref yi-%data% (kk) ((1 n)) yi-%offset%))
        (setf (f2cl-lib:fref yr-%data% (kk) ((1 n)) yr-%offset%)
