...
(let ((rtwo 1.34839972492648)
      ...
      (pp
       (make-array 4 :element-type 'double-float :initial-contents
                   '(8.72909153935547 0.26569393226503 0.124578576865586
                     7.70133747430388e-4)))
      (inlim 150)
      (fnulim
  ...
  (declare (type (double-float) rtwo pdf rttp pidt)
           (type (array double-float (4)) pp)
           (type (f2cl-lib:integer4) inlim)
           (type (array double-float (2)) fnulim))
  (defun dbesj (x alpha n y nz)
    ...
    (f2cl-lib:with-multi-array-data
      ...
      (prog ((temp (make-array 3 :element-type 'double-float))
             ...
             (tb 0.0) (tfn 0.0) (tm 0.0) (tol 0.0) (tolln 0.0) (trx 0.0)
             (tx 0.0) (t1 0.0) (t2 0.0) (xo2 0.0) (xo2l 0.0) (slim 0.0)
             (rtol 0.0) (i 0) (ialp 0) (idalp 0) (iflw 0) (in 0) (is 0) (i1 0)
             (i2 0) (k 0) (kk 0) (km 0) (kt 0) (nn 0) (ns 0))
        (declare
         (type (f2cl-lib:integer4) ns nn kt km kk k i2 i1 is in iflw idalp ialp
          i)
         (type (array double-float (7)) wk)
         (type (array double-float (3)) temp)
         (type (double-float) rtol slim xo2l xo2 t2 t1 tx trx tolln tol tm tfn
          tb tau ta t$ s2 s1 sxo2 sb sa s rzden rtx relb rden gln fnu fnp1 fni
          fnf fn flgjy fidal etx elim1 earg dtm dfn dalpha coef arg ap ans akm
          ak))
        (setf nz 0)
        (setf kt 1)
        (setf ns 0)
        ...
        (f2cl-lib:arithmetic-if (f2cl-lib:int-sub n 1) (go label720)
                                (go label10) (go label20))
       label10
        (setf kt 2)
       label20
        (setf nn n)
       ...
       label100
        ...
        (setf fnp1 (+ fn 1.0))
        (setf is kt)
        (if (> (f2cl-lib:int-add (f2cl-lib:int-sub n 1) ns) 0)
            (setf is 3))
        (go label330)
       label110
       ...
       label130
        ...
        (multiple-value-bind (var-0 var-1 var-2 var-3 var-4 var-5 var-6 var-7)
            (dasyjy #'djairy x fn flgjy i1 (f2cl-lib:fref temp (is) ((1 3))) wk
             iflw)
       ...
       label380
        ...
        (setf dfn (+ fni fnf))
        (setf fn dfn)
        (f2cl-lib:arithmetic-if (f2cl-lib:int-sub nn 1) (go label440)
                                (go label390) (go label130))
       label390
        (setf kt 2)
        (setf is 2)
        (go label130)
       ...
       label450
        ...
        (if (= kt 2)
            (go label470))
        (setf (f2cl-lib:fref y-%data% (nn) ((1 *)) y-%offset%)
                (f2cl-lib:fref temp (1) ((1 3))))
        (setf (f2cl-lib:fref y-%data% ((f2cl-lib:int-sub nn 1)) ((1 *))
                             y-%offset%)
       ...
       label455
        ...
        (f2cl-lib:fdo (i 3 (f2cl-lib:int-add i 1))
          ...
          (tagbody
            ...
            (setf tb (- (* tm tb) ta))
            (setf ta s)
            (setf (f2cl-lib:fref y-%data% (k) ((1 *)) y-%offset%) (* tb ak))
            (setf dtm (- dtm 1.0))
            (setf tm (* (+ dtm fnf) trx))
       ...
       label520
        ...
        (f2cl-lib:fdo (k 1 (f2cl-lib:int-add k 1))
          ...
          (tagbody
           ...
           label530))
       label540
        (setf (f2cl-lib:fref temp (is) ((1 3)))
                (* coef (- (* s1 sb) (* s2 sa))))
        (if (= is 2)
            (go label560))
       ...
       label560
        ...
        (setf s1 s)
       label580
        (setf (f2cl-lib:fref y-%data% (1) ((1 *)) y-%offset%) s1)
        (setf (f2cl-lib:fref y-%data% (2) ((1 *)) y-%offset%) s2)
        (if (= nn 2)
            (go end_label))
        (f2cl-lib:fdo (i 3 (f2cl-lib:int-add i 1))
                      ((> i nn) nil)
                    ...
                    (-
                     (* tm
                        (f2cl-lib:fref y-%data% ((f2cl-lib:int-sub i 1))
                                       ((1 *)) y-%offset%))
                     (f2cl-lib:fref y-%data% ((f2cl-lib:int-sub i 2)) ((1 *))
                                    y-%offset%)))
            (setf tm (+ tm tx))
           label590))
        (go end_label)
       label600
        (setf (f2cl-lib:fref y-%data% (1) ((1 *)) y-%offset%) s2)
        (go end_label)
       label610
       ...
       label620
        ...
        (if (> (f2cl-lib:fref wk (6) ((1 7))) 30.0)
            (go label640))
        (setf rden
                (+
                 (*
       ...
       label685
        ...
        (if (/= ns 0)
            (go label670))
       label690
        (setf (f2cl-lib:fref y-%data% (nn) ((1 *)) y-%offset%) (* tb ak))
        (setf nz (f2cl-lib:int-sub n nn))
        (if (= nn 1)
            (go end_label))
  ...
  (setf (gethash 'fortran-to-lisp::dbesj fortran-to-lisp::*f2cl-function-info*)
          (fortran-to-lisp::make-f2cl-finfo :arg-types
                                            ...
                                            '(fortran-to-lisp::xermsg
                                              fortran-to-lisp::dlngam
                                              fortran-to-lisp::dasyjy
