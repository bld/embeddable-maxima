...
(let ((nty0 0)
      ...
      (by0cs
       (make-array 19 :element-type 'double-float :initial-contents
                   '(-0.011277839392865573 -0.12834523756042035
                     -0.10437884799794249 0.023662749183969694
                     -0.002090391647700486 1.0397545393905725e-4
                     -3.369747162423972e-6 7.729384267670667e-8
                     -1.3249767726642596e-9 1.764823261540453e-11
                     -1.8810550715801962e-13 1.6418654853661494e-15
                     -1.1956594386046061e-17 7.377296297440186e-20
                     -3.9068434767104375e-22 1.795503664436158e-24
                     -7.22962712544801e-27 2.5717279316351685e-29
                     -8.141268814163695e-32)))
      (twodpi 0.6366197723675814)
      (first$ nil))
  (declare (type (f2cl-lib:integer4) nty0)
           (type (double-float) xsml twodpi)
           (type (array double-float (19)) by0cs)
  ...
  (defun dbesy0 (x)
    ...
    (prog ((ampl 0.0) (theta 0.0) (y 0.0) (dbesy0 0.0))
      ...
      (if (<= x 0.0)
          (xermsg "SLATEC" "DBESY0" "X IS ZERO OR NEGATIVE" 1 2))
      (if (> x 4.0)
          (go label20))
      (setf y 0.0)
      (if (> x xsml)
          (setf y (* x x)))
      (setf dbesy0
              (+ (* twodpi (f2cl-lib:flog (* 0.5 x)) (dbesj0 x)) 0.375
