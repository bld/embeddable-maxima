...
(defun zbesy (zr zi fnu kode n cyr cyi nz cwrkr cwrki ierr)
  (declare (type (array double-float (*)) cwrki cwrkr cyi cyr)
           (type (f2cl-lib:integer4) ierr nz n kode)
           (type (double-float) fnu zi zr))
  (f2cl-lib:with-multi-array-data
      ((cyr double-float cyr-%data% cyr-%offset%)
       (cyi double-float cyi-%data% cyi-%offset%)
    ...
    (prog ((i 0) (k 0) (k1 0) (k2 0) (nz1 0) (nz2 0) (c1i 0.0) (c1r 0.0)
           (c2i 0.0) (c2r 0.0) (elim 0.0) (exi 0.0) (exr 0.0) (ey 0.0)
           (hcii 0.0) (sti 0.0) (str 0.0) (tay 0.0) (ascle 0.0) (rtol 0.0)
           (atol 0.0) (aa 0.0) (bb 0.0) (tol 0.0) (r1m5 0.0))
      (declare
       (type (double-float) r1m5 tol bb aa atol rtol ascle tay str sti hcii ey
        exr exi elim c2r c2i c1r c1i)
       (type (f2cl-lib:integer4) nz2 nz1 k2 k1 k i))
      (setf ierr 0)
      (setf nz 0)
      (if (and (= zr 0.0) (= zi 0.0))
          (setf ierr 1))
      (if (< fnu 0.0)
          (setf ierr 1))
      (if (or (< kode 1) (> kode 2))
      ...
      (f2cl-lib:fdo (i 1 (f2cl-lib:int-add i 1))
        ...
        (tagbody
          (setf str
                  (- (f2cl-lib:fref cwrkr-%data% (i) ((1 n)) cwrkr-%offset%)
                     (f2cl-lib:fref cyr-%data% (i) ((1 n)) cyr-%offset%)))
          (setf sti
                  (- (f2cl-lib:fref cwrki-%data% (i) ((1 n)) cwrki-%offset%)
                     (f2cl-lib:fref cyi-%data% (i) ((1 n)) cyi-%offset%)))
          (setf (f2cl-lib:fref cyr-%data% (i) ((1 n)) cyr-%offset%)
                  (* (- sti) hcii))
          (setf (f2cl-lib:fref cyi-%data% (i) ((1 n)) cyi-%offset%)
                  (* str hcii))
         label50))
      (go end_label)
     label60
     ...
     label70
      ...
      (f2cl-lib:fdo (i 1 (f2cl-lib:int-add i 1))
                    ((> i n) nil)
        (tagbody
          (setf aa (f2cl-lib:fref cwrkr-%data% (i) ((1 n)) cwrkr-%offset%))
          (setf bb (f2cl-lib:fref cwrki-%data% (i) ((1 n)) cwrki-%offset%))
          (setf atol 1.0)
         ...
         label75
          ...
          (setf aa (f2cl-lib:fref cyr-%data% (i) ((1 n)) cyr-%offset%))
          (setf bb (f2cl-lib:fref cyi-%data% (i) ((1 n)) cyi-%offset%))
          (setf atol 1.0)
          (if (> (max (abs aa) (abs bb)) ascle)
              (go label85))
          (setf aa (* aa rtol))
