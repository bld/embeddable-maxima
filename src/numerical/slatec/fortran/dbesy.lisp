...
(let ((nulim
       (make-array 2 :element-type 'f2cl-lib:integer4 :initial-contents
                   '(70 100))))
  (declare (type (array f2cl-lib:integer4 (2)) nulim))
  (defun dbesy (x fnu n y)
    ...
    (f2cl-lib:with-multi-array-data
      ...
      (prog ((w (make-array 2 :element-type 'double-float))
             ...
             (dnu 0.0) (elim 0.0) (flgjy 0.0) (fn 0.0) (ran 0.0) (s 0.0)
             (s1 0.0) (s2 0.0) (tm 0.0) (trx 0.0) (w2n 0.0) (xlim 0.0)
             (xxn 0.0) (i 0) (iflw 0) (j 0) (nb 0) (nd 0) (nn 0) (nud 0))
        (declare (type (f2cl-lib:integer4) nud nn nd nb j iflw i)
                 (type (array double-float (7)) wk)
                 (type (array double-float (2)) w)
        ...
        (if (> cn elim)
            (go label170))
       label10
        (if (< nud (f2cl-lib:fref nulim (nn) ((1 2))))
            (go label20))
        (setf flgjy -1.0)
       ...
       label40
        ...
        (if (= nd 1)
            (setf nud (f2cl-lib:int-sub nud 1)))
        (if (> nud 0)
            (go label50))
        (if (> nd 1)
            (go label70))
       ...
       label100
        (if (<= fn 1.0)
            (go label110))
        (if (> (* (- fn) (- (f2cl-lib:flog x) 0.693)) elim)
            (go label170))
       label110
        (if (= dnu 0.0)
            (go label120))
       ...
       label120
        ...
        (setf (f2cl-lib:fref y-%data% (j) ((1 *)) y-%offset%) (dbesy0 x))
        (if (= nd 1)
            (go end_label))
        (setf j (f2cl-lib:int-add j 1))
       label130
        (setf (f2cl-lib:fref y-%data% (j) ((1 *)) y-%offset%) (dbesy1 x))
        (if (= nd 1)
            (go end_label))
        (setf trx (/ 2.0 x))
        (setf tm trx)
       ...
       label160
        ...
        (go end_label)
       label170
        (xermsg "SLATEC" "DBESY" "OVERFLOW, FNU OR N TOO LARGE OR X TOO SMALL"
         6 1)
        (go end_label)
       end_label
  ...
  (setf (gethash 'fortran-to-lisp::dbesy fortran-to-lisp::*f2cl-function-info*)
          (fortran-to-lisp::make-f2cl-finfo :arg-types
                                            ...
                                            '(fortran-to-lisp::xermsg
                                              fortran-to-lisp::dbsynu
                                              fortran-to-lisp::dbesy1
                                              fortran-to-lisp::dbesy0
