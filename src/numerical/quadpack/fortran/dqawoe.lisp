...
(defun dqawoe
  ...
  (f2cl-lib:with-multi-array-data
    ...
    (prog ((rlist2 (make-array 52 :element-type 'double-float))
           ...
           (b2 0.0) (correc 0.0) (dabs$ 0.0) (defab1 0.0) (defab2 0.0)
           (defabs 0.0) (dmax1$ 0.0) (domega 0.0) (dres 0.0) (epmach 0.0)
           (erlarg 0.0) (erlast 0.0) (errbnd 0.0) (errmax 0.0) (error1 0.0)
           (erro12 0.0) (error2 0.0) (errsum 0.0) (ertest 0.0) (oflow 0.0)
           (resabs 0.0) (reseps 0.0) (small 0.0) (uflow 0.0) (width 0.0))
      (declare (type (array double-float (52)) rlist2)
               ...
               (type (double-float) width uflow small reseps resabs oflow
                ertest errsum error2 erro12 error1 errmax errbnd erlast erlarg
                epmach dres domega dmax1$ defabs defab2 defab1 dabs$ correc b2
                b1 a2 a1 area2 area12 area1 area abseps)
               (type (f2cl-lib:integer4) numrl2 nrmom nrmax nres nev maxerr
                ktmin ksgn k jupbnd iroff3 iroff2 iroff1 ierro id)
      ...
      (setf result 0.0)
      (setf abserr 0.0)
      (setf (f2cl-lib:fref alist-%data% (1) ((1 limit)) alist-%offset%) a)
      (setf (f2cl-lib:fref blist-%data% (1) ((1 limit)) blist-%offset%) b)
      (setf (f2cl-lib:fref rlist-%data% (1) ((1 limit)) rlist-%offset%) 0.0)
      (setf (f2cl-lib:fref elist-%data% (1) ((1 limit)) elist-%offset%) 0.0)
      (setf (f2cl-lib:fref iord-%data% (1) ((1 limit)) iord-%offset%) 0)
      (setf (f2cl-lib:fref nnlog-%data% (1) ((1 limit)) nnlog-%offset%) 0)
      (if (or (and (/= integr 1) (/= integr 2))
              (and (<= epsabs 0.0)
                   (< epsrel (f2cl-lib:dmax1 (* 50.0 epmach) 5.e-29)))
              (< icall 1) (< maxp1 1))
          (setf ier 6))
      (if (= ier 6)
          (go label999))
     ...
     label5
      ...
      (setf (f2cl-lib:fref rlist-%data% (1) ((1 limit)) rlist-%offset%) result)
      (setf (f2cl-lib:fref elist-%data% (1) ((1 limit)) elist-%offset%) abserr)
      (setf (f2cl-lib:fref iord-%data% (1) ((1 limit)) iord-%offset%) 1)
      (if (and (<= abserr (* 100.0 epmach defabs)) (> abserr errbnd))
          (setf ier 2))
      (if (= limit 1)
          (setf ier 1))
      (if (or (/= ier 0) (<= abserr errbnd))
          (go label200))
      ...
      (setf iroff3 0)
      (setf ktmin 0)
      (setf small (* (f2cl-lib:dabs (- b a)) 0.75))
      (setf nres 0)
      (setf numrl2 0)
      (setf extall f2cl-lib:%false%)
      (if (> (* 0.5 (f2cl-lib:dabs (- b a)) domega) 2.0)
          (go label10))
      (setf numrl2 1)
      (setf extall f2cl-lib:%true%)
      (setf (f2cl-lib:fref rlist2 (1) ((1 52))) result)
     label10
      (if (<= (* 0.25 (f2cl-lib:dabs (- b a)) domega) 2.0)
          (setf extall f2cl-lib:%true%))
      (setf ksgn -1)
      ...
      (f2cl-lib:fdo (last$ 2 (f2cl-lib:int-add last$ 1))
        ...
        (tagbody
          (setf nrmom
                  (f2cl-lib:int-add
                   (f2cl-lib:fref nnlog-%data% (maxerr) ((1 limit))
                                  nnlog-%offset%)
                   1))
          (setf a1
                  (f2cl-lib:fref alist-%data% (maxerr) ((1 limit))
                                 alist-%offset%))
          (setf b1
                  (* 0.5
                     (+
                      (f2cl-lib:fref alist-%data% (maxerr) ((1 limit))
                                     alist-%offset%)
                      (f2cl-lib:fref blist-%data% (maxerr) ((1 limit))
                                     blist-%offset%))))
          (setf a2 b1)
          (setf b2
                  (f2cl-lib:fref blist-%data% (maxerr) ((1 limit))
          ...
          (setf area
                  (- (+ area area12)
                     (f2cl-lib:fref rlist-%data% (maxerr) ((1 limit))
                                    rlist-%offset%)))
          (if (or (= defab1 error1) (= defab2 error2))
              (go label25))
          (if (or
               (>
                (f2cl-lib:dabs
         ...
         label25
          (setf (f2cl-lib:fref rlist-%data% (maxerr) ((1 limit))
                               rlist-%offset%)
                  area1)
          (setf (f2cl-lib:fref rlist-%data% (last$) ((1 limit)) rlist-%offset%)
                  area2)
          (setf (f2cl-lib:fref nnlog-%data% (maxerr) ((1 limit))
                               nnlog-%offset%)
                  nrmom)
          (setf (f2cl-lib:fref nnlog-%data% (last$) ((1 limit)) nnlog-%offset%)
                  nrmom)
          ...
          (if (= last$ limit)
              (setf ier 1))
          (if (<= (f2cl-lib:dmax1 (f2cl-lib:dabs a1) (f2cl-lib:dabs b2))
                  (* (+ 1.0 (* 100.0 epmach))
                     (+ (f2cl-lib:dabs a2) (* 1000.0 uflow))))
              (setf ier 4))
          (if (> error2 error1)
              (go label30))
          (setf (f2cl-lib:fref alist-%data% (last$) ((1 limit)) alist-%offset%)
                  a2)
          (setf (f2cl-lib:fref blist-%data% (maxerr) ((1 limit))
                               blist-%offset%)
                  b1)
          (setf (f2cl-lib:fref blist-%data% (last$) ((1 limit)) blist-%offset%)
                  b2)
          (setf (f2cl-lib:fref elist-%data% (maxerr) ((1 limit))
                               elist-%offset%)
                  error1)
          (setf (f2cl-lib:fref elist-%data% (last$) ((1 limit)) elist-%offset%)
                  error2)
          (go label40)
         label30
          (setf (f2cl-lib:fref alist-%data% (maxerr) ((1 limit))
                               alist-%offset%)
                  a2)
          (setf (f2cl-lib:fref alist-%data% (last$) ((1 limit)) alist-%offset%)
                  a1)
         ...
         label70
          ...
          (f2cl-lib:fdo (k id (f2cl-lib:int-add k 1))
            ...
            (tagbody
              (setf maxerr
                      (f2cl-lib:fref iord-%data% (nrmax) ((1 limit))
                                     iord-%offset%))
              (setf errmax
                      (f2cl-lib:fref elist-%data% (maxerr) ((1 limit))
                                     elist-%offset%))
              (if (>
                   (f2cl-lib:dabs
                    (-
         ...
         label90
          ...
          (if (and (> ktmin 5) (< abserr (* 0.001 errsum)))
              (setf ier 5))
          (if (>= abseps abserr)
              (go label100))
          (setf ktmin 0)
         ...
         label140))
     label150
      (if (or (= abserr oflow) (= nres 0))
          (go label170))
      (if (= (f2cl-lib:int-add ier ierro) 0)
          (go label165))
      (if (= ierro 3)
          (setf abserr (+ abserr correc)))
      (if (= ier 0)
          (setf ier 3))
      (if (and (/= result 0.0) (/= area 0.0))
          (go label160))
      (if (> abserr errsum)
          (go label170))
      (if (= area 0.0)
     ...
     label165
      (if (and (= ksgn -1)
               (<= (f2cl-lib:dmax1 (f2cl-lib:dabs result) (f2cl-lib:dabs area))
                   (* defabs 0.01)))
          (go label190))
      (if (or (> 0.01 (/ result area)) (> (/ result area) 100.0)
     ...
     label170
      ...
      (f2cl-lib:fdo (k 1 (f2cl-lib:int-add k 1))
        ...
        (tagbody
          (setf result
                  (+ result
                     (f2cl-lib:fref rlist-%data% (k) ((1 limit))
                                    rlist-%offset%)))
         label180))
      (setf abserr errsum)
     label190
      (if (> ier 2)
          (setf ier (f2cl-lib:int-sub ier 1)))
     label200
      (if (and (= integr 2) (< omega 0.0))
          (setf result (- result)))
     label999
      (go end_label)
     end_label
      (return
       (values nil nil nil nil integr nil nil nil nil nil result abserr neval
               ier last$ nil nil nil nil nil nil momcom nil)))))
  ...
  (setf (gethash 'fortran-to-lisp::dqawoe
          ...
          (fortran-to-lisp::make-f2cl-finfo :arg-types
                                            ...
                                            '(nil nil nil nil
                                              ...
                                              nil nil nil nil
                                              fortran-to-lisp::momcom nil)
                                            :calls
                                            '(fortran-to-lisp::dqelg
                                              fortran-to-lisp::dqpsrt
                                              fortran-to-lisp::dqc25f
